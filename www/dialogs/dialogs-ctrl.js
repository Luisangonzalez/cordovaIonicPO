(function() {
  'use strict';

  angular
    .module('starter')
    .controller('DialogsCtrl', ['$ionicPlatform', '$cordovaDialogs', '$cordovaVibration', DialogsCtrl]);

  // DialogsCtrl.$inject = ['ngCordova'];

  /* @ngInject */
  function DialogsCtrl($ionicPlatform, $cordovaDialogs, $cordovaVibration) {
    var vm = this;

    activate();

    function activate() {
      console.log('DialogsCtrl');
      vm.hello = 'Hello'
    }

    vm.alert = function() {
      $cordovaDialogs.alert('Wow!', alertClosed, "Alert Title", "Dismiss");
    };

    function alertClosed() {
      $cordovaDialogs.alert("Alert was closed.");
    }

    vm.confirm = function() {
      $cordovaDialogs.confirm('Are you sure?', confirmClosed, "Confirmation", ["Yes", "No"]);
    };

    function confirmClosed(buttonIndex) {
      $cordovaDialogs.alert("Button selected: " + buttonIndex);
    }

    vm.prompt = function() {
      $cordovaDialogs.prompt('Please Login', promptClosed, "Login", ["Login", "Cancel"]);
    };

    function promptClosed(results) {
      alert("You selected button number " + results.buttonIndex + " and entered " + results.input1);
    }

    vm.beep = function() {
      document.addEventListener("deviceready", function() {
        $cordovaDialogs.beep(3);
      }, false);

    };

    vm.vibrate = function() {
      document.addEventListener("deviceready", function() {
        $cordovaVibration.vibrate(2000);
      }, false);

    };



  }
})();
